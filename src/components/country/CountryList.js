...
import bg from "../../lib/util/bg.mp3";
...
class CountryList extends Component {
    render() {
        ...
        const map1List = reversemap1.map((info, i) => <Country key={i} {...info} location={player[turn].location} userid={player[turn].userid} collected={collected} />);
        const map2List = map2.map((info, i) => <Country key={i} {...info} location={player[turn].location} userid={player[turn].userid} collected={collected} />);
        const map3List = map3.map((info, i) => <Country key={i} {...info} location={player[turn].location} userid={player[turn].userid} collected={collected} />);
        const map4List = reversemap4.map((info, i) => <Country key={i} {...info} location={player[turn].location} userid={player[turn].userid} collected={collected} />);

        return (
            <div>
                {/* <audio src={bg} autoPlay={true} loop={true}></audio> */}
                {/* <UpPlayerInfoContainer /> */}
                <div className="CountryList clearfix">
                    <div className="first_line clearfix">{map2List}</div>
                    <div className="second_line clearfix">{map1List}</div>
                    <CountryInfoContainer />
                    <DiceContainer />
                    <div className="third_line clearfix">{map3List}</div>
                    <div className="four_line clearfix">{map4List}</div>
                </div>
